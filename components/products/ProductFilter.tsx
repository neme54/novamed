      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="relative w-full lg:max-w-xl" id="search">
          ...
          <input
            ...
            placeholder={pages.products.searchPlaceholder}
            aria-label={pages.products.searchPlaceholder}
          ...
          {q ? (
            <button
              type="button"
              aria-label={pages.products.clearFilters}
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <label className="text-sm font-semibold text-dark">
            {pages.products.categoryLabel}
          </label>
          <select
            value={category}
            onChange={(e) => update({ category: e.target.value || null })}
            className="h-12 rounded-2xl border border-black/10 bg-surface px-4 text-sm text-dark focus-visible:focus-ring"
            aria-label={pages.products.categoryLabel}
          >
            <option value="">{pages.products.typeTabs[0]?.label}</option>
            {categories.map((c) => (
          ...
          >
            {pages.products.clearFilters}
          </Button>
        </div>
      </div>

      <div className="mt-5 flex flex-wrap gap-2">
        {pages.products.typeTabs.map((t) => (
          <button
            key={t.key}
            type="button"
            ...
            className={cn(
              ...
              type === t.key || (t.key === "all" && type === "all")
                ? "bg-primary text-surface"
                : "bg-surface text-dark hover:bg-highlight",
            )}
            aria-pressed={type === t.key || (t.key === "all" && type === "all")}
          >
            {t.label}
          </button>
        ))}
